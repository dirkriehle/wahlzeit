...
import static org.wahlzeit.services.CloudDataBase.getOpActions;
...
public class GlobalsManager extends ObjectManager {
    ...
    private boolean hasGlobals() {
        return ObjectifyService.run(new Work<Boolean>() {
            @Override
            public Boolean run() {
                return getOpActions().load().type(Globals.class).first().now() != null;
            }
        });
    }
    ...
    private void createDefaultGlobals() {
        ObjectifyService.run(new Work<Boolean>() {
            ...
            public Boolean run() {
                ...
                getOpActions().save().entity(globals).now();
                return null;
            }
        });
    }
